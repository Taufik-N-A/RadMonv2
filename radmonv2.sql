...
CREATE TRIGGER `before_radpostauth_insert` BEFORE INSERT ON `radpostauth` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `insert_income` AFTER INSERT ON `radpostauth` FOR EACH ROW
BEGIN
    DECLARE plan_cost DECIMAL(10, 2);

    IF NEW.reply = 'log in by voucher' OR NEW.reply = 'log in by mac' THEN
        SELECT bp.planCost
        INTO plan_cost
        ...
        IF plan_cost IS NOT NULL AND plan_cost > 0 THEN
